...
這是第一版（Part 1）瀏覽器擴充功能，目標是**在 YouTube 播放時自動略過可略過的廣告**，並盡量以「幾乎無感」方式處理廣告（優先直接跳到廣告結尾，失敗時再用靜音備援）。

## 功能

- 自動偵測 YouTube 是否處於廣告播放狀態
- 優先嘗試「直接跳到廣告結尾」（非快進，接近無感切換）
- 若可略過按鈕出現，會自動點擊「略過廣告」
- 若某些廣告型態無法立即跳轉，則暫時靜音作為備援
- 廣告結束後自動恢復音量狀態（如果是擴充功能改成靜音，會自動解除）
...
3. 觀察下列行為是否出現：
   - 有廣告時，多數情況會快速切回正片（幾乎不會感覺快進）。
   - 廣告可略過時，按鈕出現後會自動點擊。
   - 若該廣告無法直接跳轉，播放器會暫時靜音，結束後恢復。
